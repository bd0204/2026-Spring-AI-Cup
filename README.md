...
分析:
有幾項有用的改動:
起初只有將match與id進行30%的隨機設定為0，當時的模型分數LB為0.45，後續將id與match移除後，LB為0.50，有0.05的提升，
再來將id改為沒看過遮蔽時，提升0.01。

改進方向:
1. 我們利用生成式AI給予特徵的方向，因此都是一個區塊一個區塊的加入特徵，例如連續性、戰術、落點位置、落點動作統計等等。
因為繳交次數有限，並沒有一個特徵一個特徵的拆開來重新訓練，只看一整組是否有在LB、自己的OOF收到效果，未來可以將特徵分群進一步縮小，進而過濾掉包含在大群中的雜訊特徵。
